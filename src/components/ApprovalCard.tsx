...
export function ApprovalCard({ bridge, approval }: { bridge: Bridge; approval: Approval }) {
  ...
  return (
    <section className="bg-surface-container-low rounded-xl p-4 border-l-4 border-secondary space-y-3 min-w-0" role="alert" aria-label={isQuestion ? 'Worker question' : 'Approval required'}>
        ...
        <>
          {isWrite ? (
            approval.preview ? <div className="space-y-2 min-w-0">
              ...
              <details className="text-body-sm"><summary className="cursor-pointer">Full proposed file ({approval.preview.bytes} bytes)</summary><pre aria-label="Proposed file contents" className="mt-2 max-h-80 overflow-auto whitespace-pre p-3 rounded-lg bg-surface-container-lowest font-mono" tabIndex={0}>{approval.preview.content}</pre></details>
            </div> : <p className="text-body-sm text-on-surface-variant" role="status">Loading proposed changes before approval…</p>
          ) : <pre aria-label="Resolved tool arguments" className="p-3 rounded-lg bg-surface-container-lowest font-mono text-body-sm whitespace-pre-wrap break-all max-h-80 overflow-auto" tabIndex={0}>{JSON.stringify(call.resolved ?? call.input, null, 2)}</pre>}
          ...
          {showDeny ? (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button ref={denyButton} type="button" disabled={busy} className="ml-auto px-3 py-2 rounded-lg text-label-md text-error hover:bg-error-container/30 disabled:opacity-40" onClick={() => setShowDeny(true)}>Deny <kbd>d</kbd></button>
            </div>
          )}
        </>
      )}
